...
export async function createOrder(formData: FormData) {
  ...
  try {
    ...
    // ✅ VERIFICAR STOCK (productos simples y variantes)
    for (const item of items) {
      if (item.variantId) {
        ...
        if (!variant || variant.stock < item.quantity) {
          return { error: `Stock insuficiente para: ${item.name} (${item.variantColor || ""})` }
        }
      } else {
        // Es producto simple
    ...
    for (const item of items) {
      await supabase.from("order_items").insert({
        ...
        // ✅ AGREGADO: guardar datos de variante
      ...
      if (item.variantId) {
        // ✅ Descontar stock de la variante vía RPC atómica
        await supabase.rpc("decrement_variant_stock", {
          p_variant_id: item.variantId,
          p_quantity: item.quantity,
        })
      } else {
        // Producto simple: seguir como antes
